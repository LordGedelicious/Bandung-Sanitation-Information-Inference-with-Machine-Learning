...
for row_id in range(total_rows):
    ...
    tangkiSeptik_penggunaan = np.random.choice(["Berfungsi","Tidak Berfungsi"])
    tangkiSeptik_penggunaan_num = 1 if tangkiSeptik_penggunaan == "Berfungsi" else 0
    tangkiSeptik_terawat = np.random.choice(["Ya","Tidak"])
    tangkiSeptik_terawat_num = 1 if tangkiSeptik_terawat == "Ya" else 0
    tangkiSeptik_bauTidakSedap = np.random.choice(["Ya","Tidak"])
    tangkiSeptik_bauTidakSedap_num = 0 if tangkiSeptik_bauTidakSedap == "Ya" else 1
    tangkiSeptik_ketercemaranLingkungan = np.random.choice(["Ya","Tidak"])
    ...
    iplt_penggunaan = np.random.choice(["Berfungsi","Tidak Berfungsi"])
    iplt_penggunaan_num = 1 if iplt_penggunaan == "Berfungsi" else 0
    iplt_bangunanTerawat = np.random.choice(["Ya","Tidak"])
    iplt_bangunanTerawat_num = 1 if iplt_bangunanTerawat == "Ya" else 0
    iplt_bauTidakSedap = np.random.choice(["Ya","Tidak"])
    iplt_bauTidakSedap_num = 0 if iplt_bauTidakSedap == "Ya" else 1
    iplt_pencemaranLingkungan = np.random.choice(["Ya","Tidak"])
    iplt_pencemaranLingkungan_num = 0 if iplt_pencemaranLingkungan == "Ya" else 1
    iplt_airLimbahBerwarna = np.random.choice(["Ya","Tidak"])
    ...
    saluranDrainase_hierarki = np.random.choice(["Primer","Sekunder","Tersier"])
    saluranDrainase_hierarki_num = 0.05
    saluranDrainase_jenis = np.random.choice(["Terbuka","Tertutup"])
    saluranDrainase_jenis_num = 0.1
    saluranDrainase_bentukPenampang = np.random.choice(["Persegi", "Trapesiun", "Setengah Lingkaran", "Segitiga"])
    saluranDrainase_bentukPenampang_num = 0.05
    saluranDrainase_perkerasanTepi = np.random.choice(["Pasir Halus","Lempung Kepasiran","Tanah","Kerikil Halus","Batuan"])
    saluranDrainase_perkerasanTepi_num = 0.1 if saluranDrainase_perkerasanTepi in ["Kerikil Halus", "Batuan"] else 0
    saluranDrainase_kondisiDrainase = np.random.choice(["Terawat","Tidak Terawat"])
    saluranDrainase_kondisiDrainase_num = 0.3 if saluranDrainase_kondisiDrainase == "Terawat" else 0
    saluranDrainase_bauTidakSedap = np.random.choice(["Ya","Tidak"])
    saluranDrainase_bauTidakSedap_num = 0 if saluranDrainase_bauTidakSedap == "Ya" else 0.1
    saluranDrainase_sedimentasiDrainase = np.random.choice(["Tidak Ada","Tanah","Sampah","Eceng Gondok"])
...
sni_df.to_csv("data/sni_df.csv", sep=",", index=False)
print("Data created!")